...
const testObj = {
  ...
  goDwon() {
    ...
    optionsElement.value[this.index].focus();
  },
  goUp() {
    this.index - 1;
    if (this.index == -1) {
...
const f = () => {
  // console.log(optionsElement.value[0].focus());
  console.log('f2in');
};
const f2 = () => {
  console.log('click');
...
<template>
  <div
    tabindex="0"
    style="width: 300px; position: relative; flex-direction: column"
    class="d-flex"
    @blur="b"
  >
    <div class="d-flex test" @focusin="f">
      <input
        :class="state.status"
        class="searching"
        ...
        :value="inputValue"
        type="text"
        @keydown.down="testObj.goDwon()"
        @keydown.up="testObj.goUp()"
    ...
    <div class="list" :id="idBase + 'select'" v-show="showTime">
      <template v-for="index in 5" :key="index">
        <div class="border w-100 option" ref="optionsElement" @click="f2">
          {{ index + 'hahaha' }}
        </div>
      </template>
    </div>
...
.test::after {
  &:focus-within {
    ...
    border: 30px dodgerblue solid;
    box-shadow: 1px 1px 1px 3px rgba(30, 143, 255, 0.297);
  }
}
